...
use ag_ui_core::{BaseEventFields, Event, Message, RunAgentInput, RunFinishedEvent, TextMessageChunkEvent, TextMessageRole};
...
async fn outer_and_inner_state_tracking_middlewares_both_capture_messages() {
    ...
    let agent = FakeAgent {
        events: vec![
            ag_ui_core::factory::run_started("test-thread", "test-run"),

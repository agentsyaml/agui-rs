...
use ag_ui_core::{Event, Result, RunAgentInput, RunFinishedEvent};
...
impl Agent for EchoAgent {
    async fn run(&self, input: RunAgentInput) -> Result<BoxStream<'static, Result<Event>>> {
        ...
        Ok(Box::pin(stream::iter(vec![
            Ok(ag_ui_core::factory::run_started(&thread_id, &run_id)),
            Ok(Event::TextMessageChunk(ag_ui_core::TextMessageChunkEvent {
            ...
            Ok(Event::RunFinished(RunFinishedEvent {
                ...
                run_id,
                result: None,
                outcome: Some(ag_ui_core::RunFinishedOutcome::Success),
                base: ag_ui_core::BaseEventFields::default(),
            })),
        ])))
    }
...
async fn concurrent_runs_preserve_per_run_message_content() {
    ...
    match &result_a.new_messages[0] {
        ag_ui_core::Message::Assistant(AssistantMessage { content, .. }) => {
...
// SKIPPED: run_cancellable concurrency cannot be integration-tested through the public crate API because AbortHandle is not publicly re-exported from ag_ui_client.

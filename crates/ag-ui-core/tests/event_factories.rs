use ag_ui_core::event_factories as factory;
use ag_ui_core::{
    Event, ReasoningEncryptedValueSubtype, ReasoningMessageRole, RunFinishedOutcome,
    TextMessageRole, ToolResultRole,
};
use ag_ui_core::types::{Interrupt, Message, RunAgentInput};
use serde_json::json;

#[test]
fn text_message_factories_cover_roles_names_and_optional_fields() {
    let start_default = factory::create_text_message_start_event("msg-1", None, None, Some(11), None);
    ...
    let chunk_optional = factory::create_text_message_chunk_event(None, None, Some("partial".into()), None, Some(17), None);

    assert!(matches!(start_default, Event::TextMessageStart(ref event) if event.role == TextMessageRole::Assistant && event.base.timestamp == Some(11)));
    assert!(matches!(start_user, Event::TextMessageStart(ref event) if event.role == TextMessageRole::User && event.name.as_deref() == Some("research-agent")));
    assert!(matches!(content_empty, Event::TextMessageContent(ref event) if event.delta.is_empty()));
    ...
    assert!(matches!(chunk_named, Event::TextMessageChunk(ref event) if event.name.as_deref() == Some("research-agent")));
    assert!(matches!(chunk_optional, Event::TextMessageChunk(ref event) if event.delta.as_deref() == Some("partial") && event.message_id.is_none()));
}

#[test]
fn thinking_message_and_reasoning_factories_build_expected_variants() {
    let thinking_start = factory::create_thinking_start_event(Some("working".into()), Some(21), None);
    let thinking_text_start = factory::create_thinking_text_message_start_event(Some(22), None);
    let thinking_text_content = factory::create_thinking_text_message_content_event("thinking…", Some(23), None);
    ...
    let reasoning_message_content = factory::create_reasoning_message_content_event("r1", "step", Some(28), None);
    let reasoning_message_end = factory::create_reasoning_message_end_event("r1", Some(29), None);
    let reasoning_chunk = factory::create_reasoning_message_chunk_event(Some("r1".into()), Some("chunk".into()), Some(30), None);
    ...
    assert!(matches!(thinking_start, Event::ThinkingStart(ref event) if event.title.as_deref() == Some("working")));
    assert!(matches!(thinking_text_start, Event::ThinkingTextMessageStart(_)));
    assert!(matches!(thinking_text_content, Event::ThinkingTextMessageContent(ref event) if event.delta == "thinking…"));
    assert!(matches!(thinking_text_end, Event::ThinkingTextMessageEnd(_)));
    assert!(matches!(thinking_end, Event::ThinkingEnd(_)));
    assert!(matches!(reasoning_start, Event::ReasoningStart(ref event) if event.message_id == "r1"));
    assert!(matches!(reasoning_message_start, Event::ReasoningMessageStart(ref event) if event.role == ReasoningMessageRole::Reasoning));
    assert!(matches!(reasoning_message_content, Event::ReasoningMessageContent(ref event) if event.delta == "step"));
    assert!(matches!(reasoning_message_end, Event::ReasoningMessageEnd(ref event) if event.message_id == "r1"));
    assert!(matches!(reasoning_chunk, Event::ReasoningMessageChunk(ref event) if event.delta.as_deref() == Some("chunk")));
    assert!(matches!(reasoning_end, Event::ReasoningEnd(ref event) if event.message_id == "r1"));
    assert!(matches!(encrypted, Event::ReasoningEncryptedValue(ref event) if event.subtype == ReasoningEncryptedValueSubtype::ToolCall));
}
...
fn tool_call_factories_cover_start_args_chunk_end_and_result() {
    ...
    assert!(matches!(start, Event::ToolCallStart(ref event) if event.parent_message_id.as_deref() == Some("msg-parent")));
    assert!(matches!(args, Event::ToolCallArgs(ref event) if event.delta.contains('q')));
    assert!(matches!(chunk, Event::ToolCallChunk(ref event) if event.delta.as_deref() == Some("partial")));
    assert!(matches!(end, Event::ToolCallEnd(ref event) if event.tool_call_id == "tc-1"));
    assert!(matches!(result, Event::ToolCallResult(ref event) if event.role == Some(ToolResultRole::Tool) && event.content == r#"{"ok":true}"#));
}

#[test]
fn snapshot_raw_custom_run_and_step_factories_cover_public_api() {
    let messages = vec![
        serde_json::from_value::<Message>(json!({ "role": "user", "id": "u1", "content": "hello" })).unwrap(),
        serde_json::from_value::<Message>(json!({ "role": "assistant", "id": "a1", "content": "hi there" })).unwrap(),
    ];
    let run_input = RunAgentInput::new("t1", "r1");
    let state_snapshot = factory::create_state_snapshot_event(json!({ "step": 1 }), Some(51), None);
    let state_delta = factory::create_state_delta_event(vec![json!({ "op": "add", "path": "/foo", "value": "bar" })], Some(52), None);
    ...
    let raw = factory::create_raw_event(json!({ "any": true }), Some("webhook".into()), Some(55), None);
    let custom = factory::create_custom_event("metric", json!(42), Some(56), None);
    let started = factory::create_run_started_event("t1", "r1", None, Some(run_input), Some(57), None);
    let finished = factory::create_run_finished_event("t1", "r1", Some(json!({ "ok": true })), None, Some(58), None);
    ...
    assert!(matches!(state_snapshot, Event::StateSnapshot(ref event) if event.snapshot == json!({ "step": 1 })));
    assert!(matches!(state_delta, Event::StateDelta(ref event) if event.delta[0]["op"] == "add"));
    assert!(matches!(messages_snapshot, Event::MessagesSnapshot(ref event) if event.messages.len() == 2 && event.base.timestamp == Some(123)));
    assert!(matches!(activity_snapshot, Event::ActivitySnapshot(ref event) if event.replace));
    assert!(matches!(activity_delta, Event::ActivityDelta(ref event) if event.patch[0]["path"] == "/steps/0"));
    ...
    assert!(matches!(started, Event::RunStarted(ref event) if event.input.as_ref().map(|input| input.run_id.as_str()) == Some("r1")));
    assert!(matches!(finished, Event::RunFinished(ref event) if event.result == Some(json!({ "ok": true })) && event.outcome.is_none()));
...
fn run_finished_success_factory_sets_success_outcome() {
    let event = factory::create_run_finished_success_event("t-1", "r-1", Some(json!({ "ok": true })), Some(71), None);

    assert!(matches!(event, Event::RunFinished(ref event) if event.outcome == Some(RunFinishedOutcome::Success) && event.result == Some(json!({ "ok": true }))));
}
...
fn run_finished_factory_accepts_explicit_success_and_interrupt_outcomes() {
    ...
    assert!(matches!(success, Event::RunFinished(ref event) if event.outcome == Some(RunFinishedOutcome::Success)));
    assert!(matches!(interrupt, Event::RunFinished(ref event) if matches!(&event.outcome, Some(RunFinishedOutcome::Interrupt { interrupts }) if interrupts.len() == 1)));
}
